choose-language = Выберите язык:
ask-name = Введите ваше имя:
ask-phone = Отправьте ваш номер телефона:
share-phone-button = 📱 Отправить номер
registration-done = Вы успешно зарегистрированы! ✅
welcome-back = Добро пожаловать, { $name }!
invalid-phone = Пожалуйста, отправьте номер через кнопку.

# Кнопки главного меню
menu-courses = 📚 Курсы
menu-tests = 🎯 Тесты
menu-payments = 💳 Оплата
menu-homework = 📋 Домашнее задание
menu-schedule = 📅 Расписание
menu-profile = 👤 Мой профиль
menu-progress = 📊 Прогресс
menu-ranking = 🏆 Рейтинг
menu-referral = 👥 Реферал
menu-language = 🌐 Язык
menu-contact = 📞 Контакты
menu-free-lesson = 📝 Запись на бесплатный урок

coming-soon = 🔧 Этот раздел скоро будет доступен.

open-test-prompt = Нажмите кнопку ниже, чтобы начать тест:
open-test-button = 🎯 Начать тест